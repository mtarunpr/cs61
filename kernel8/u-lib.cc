...
// sys_spawn(program_name)
//    Start a new process running the `program_name` process image.
//    Returns the new process’s process ID or a negative error code.
__noinline pid_t sys_spawn(const char* program_name) {
    return make_syscall(SYSCALL_SPAWN, (uintptr_t) program_name);
}
